    ...
    if not PUBLIC_LINK_BUTTON_DATA:
        ...
        for row_idx, row in enumerate(PUBLIC_LINK_BUTTON_DATA, start=1):
            ...
            for col_idx, item in enumerate(row, start=1):
                text += f"{row_idx}.{col_idx} {item.get('text', 'Link')} → {item.get('url', '')}\n"
            text += "\n"
...
async def admin_links_add_save(message: types.Message, state: FSMContext):
    ...
    await message.answer(format_public_links_admin_text(), parse_mode="Markdown", reply_markup=build_public_links_admin_kb())
...
async def admin_links_delete(cb: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await cb.message.edit_text(format_public_links_admin_text(), reply_markup=build_public_links_admin_kb(), parse_mode="Markdown")
    except Exception as e:
...
async def admin_links_edit_save(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer("✅ Link button updated.", parse_mode="Markdown")
    await message.answer(format_public_links_admin_text(), parse_mode="Markdown", reply_markup=build_public_links_admin_kb())
