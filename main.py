...
def refresh_broadcast_users_cache() -> set[str]:
    """Reload broadcast users from database/history and keep memory cache in sync."""
    global all_users
    try:
        if db:
            try:
                db.restore_all_users_from_history()
            except Exception as restore_error:
                logger.error(f"❌ Error restoring users from history: {restore_error}")
            fresh_users = db.get_broadcast_users()
            if fresh_users:
                all_users = set(str(uid) for uid in fresh_users)
        return all_users
    except Exception as e:
        logger.error(f"❌ Error refreshing broadcast users cache: {e}")
        return all_users


def get_broadcast_recipients() -> list[int]:
    """Return clean integer user ids for broadcast sending."""
    recipients = []
    for user_id in refresh_broadcast_users_cache():
        try:
            user_id_int = int(user_id)
            if user_id_int > 0 and str(user_id_int) != ADMIN_ID:
                recipients.append(user_id_int)
        except (TypeError, ValueError):
            logger.warning(f"⚠️ Skipping invalid broadcast user id: {user_id}")
    return recipients


def get_total_broadcast_users_count() -> int:
    """Get the most accurate broadcast user count available."""
    try:
        if db:
            count = db.get_broadcast_user_count()
            if isinstance(count, int) and count >= 0:
                return count
    except Exception as e:
        logger.error(f"❌ Error getting broadcast user count: {e}")
    return len(refresh_broadcast_users_cache())


def count_products_in_menu_data(menu_data: dict) -> int:
    """Count products safely from db.get_dynamic_menu() response."""
    if not isinstance(menu_data, dict):
        return 0
    menu = menu_data.get('menu', menu_data)
    if not isinstance(menu, dict):
        return 0
    return sum(len(products_in_category) for products_in_category in menu.values() if isinstance(products_in_category, dict))
...
async def admin_broadcast_send(message: types.Message, state: FSMContext):
    ...
    status_message = await message.answer(f"📢 Broadcasting message... 0/{total_recipients}")
    ...
    for i, user_id in enumerate(recipients, 1):
        try:
            ...
            if i % 10 == 0 or i == total_recipients:
                try:
                    await status_message.edit_text(f"📢 Broadcasting... {i}/{total_recipients}")
                except:
                    pass
    ...
    result_text += f"👥 Total users: {total_recipients}\n"
...
async def add_category_process(message: types.Message, state: FSMContext):
    ...
    total_products = count_products_in_menu_data(menu_data)
...
async def admin_stats(cb: types.CallbackQuery, state: FSMContext):
    ...
    # Calculate stats
    memory_users = len(refresh_broadcast_users_cache())
