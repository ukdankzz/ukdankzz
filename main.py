...
# BOT_START_AUTHORIZED guard removed – not needed on Railway.
# Railway uses a Procfile / start command so there is no duplicate-startup risk.
...
try:
    import fcntl
except ImportError:  # Windows/local testing support; Railway/Linux still uses real locks.
    fcntl = None
import signal
import atexit
import time
...
# Start bundled catalog items inactive by default when requested.
# Admin can turn products back on from Admin Panel → Toggle Product Status.
BUNDLED_PRODUCT_NAMES = [
    product_name
    for category_products in ORIGINAL_PRODUCTS.values()
    for product_name in category_products.keys()
]
AUTO_DISABLE_BUNDLED_PRODUCTS_ON_STARTUP = os.getenv(
    "AUTO_DISABLE_BUNDLED_PRODUCTS_ON_STARTUP", "1"
...
try:
    if db:
        ...
        if AUTO_DISABLE_BUNDLED_PRODUCTS_ON_STARTUP and hasattr(db, 'set_products_active_by_name'):
            disabled_count = db.set_products_active_by_name(BUNDLED_PRODUCT_NAMES, active=False)
            print(f"🔒 Bundled startup catalog disabled by default: {disabled_count} products toggled off")
        ...
        products = {} if AUTO_DISABLE_BUNDLED_PRODUCTS_ON_STARTUP else ORIGINAL_PRODUCTS
except Exception as e:
...
async def add_category_process(message: types.Message, state: FSMContext):
    ...
    menu_data = db.get_dynamic_menu() if db else {'menu': {}}
    menu = menu_data.get('menu', {}) if isinstance(menu_data, dict) else {}
    total_products = sum(len(prods) for prods in menu.values())
...
async def select_category_for_product(cb: types.CallbackQuery, state: FSMContext):
    ...
    text += "Step 2: Type the product name, or add multiple products at once.\n\n"
    text += "Single product example: `CBD Oil`\n\n"
    text += "Multi-add format, one product per line:\n"
    text += "`CBD Oil | 500mg 25 | 1000mg 45`\n"
    text += "`CBD Gummies | 1 15 | 3 40`"
...
async def add_product_name(message: types.Message, state: FSMContext):
    ...
    # Professional bulk add: Name | size price | size price
    # Example: CBD Oil | 500mg 25 | 1000mg 45
    if "|" in message.text and "\n" in message.text.strip():
        ...
        def parse_bulk_tier(tier_text: str):
            tier_text = tier_text.strip().replace('£', '').replace('$', '').replace('€', '')
            if ' for ' in tier_text.lower():
                left, right = tier_text.lower().split(' for ', 1)
                return left.strip(), float(right.strip())
            parts = tier_text.split()
            if len(parts) < 2:
                raise ValueError('tier must be: size price')
            return parts[0].strip(), float(parts[1].strip())

        for raw_line in message.text.strip().splitlines():
            line = raw_line.strip()
            if not line:
                continue
            try:
                ...
                if len(parts) < 2:
                    raise ValueError('use: Product Name | size price | size price')
                bulk_product_name = parts[0][:200]
                pricing_tiers = [parse_bulk_tier(part) for part in parts[1:]]
                prod_result = db.add_product(category_id, bulk_product_name)
                if not prod_result.get('success'):
                    raise ValueError(prod_result.get('error', 'product add failed'))
                price_result = db.update_product_pricing(category, bulk_product_name, pricing_tiers)
                if not price_result.get('success'):
                    raise ValueError(price_result.get('error', 'pricing add failed'))
                added.append(bulk_product_name)
            except Exception as exc:
                failed.append(f"{line} — {exc}")

        global products, descriptions
        menu_data = db.get_dynamic_menu()
        products = menu_data.get('menu', {})
        descriptions.update(menu_data.get('descriptions', {}))

        reply = "✅ *Bulk Add Complete*\n\n"
        if added:
...
async def main():
    ...
    try:
        if fcntl is not None:
            ...
            print("⚠️ fcntl unavailable on this OS; skipping process lock for local testing.")
    except IOError:
