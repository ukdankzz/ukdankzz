...
async def admin_links_add_save(message: types.Message, state: FSMContext):
    ...
    if not label or not url:
        await message.answer("❌ Invalid format. Use:\n`Button Name | https://t.me/yourlink`", parse_mode="Markdown")
        return
    ...
    await state.clear()
    await message.answer("✅ Link button added.", reply_markup=build_public_links_admin_kb())
    await message.answer(format_public_links_admin_text(), reply_markup=build_public_links_admin_kb())
...
async def admin_links_delete(cb: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await cb.message.edit_text(format_public_links_admin_text(), reply_markup=build_public_links_admin_kb())
    except Exception as e:
...
async def admin_links_edit_save(message: types.Message, state: FSMContext):
    ...
    if not save_public_link_button_data(new_data):
        await message.answer("❌ Failed to save updated link.")
        return

    await state.clear()
    await message.answer("✅ Link button updated.")
    await message.answer(format_public_links_admin_text(), reply_markup=build_public_links_admin_kb())
...
async def show_product_reviews(cb: types.CallbackQuery, state: FSMContext):
    ...
    if not product_specific_reviews:
        ...
        for review in product_specific_reviews:
            ...
            if len(safe_review_text) > 280:
                safe_review_text = safe_review_text[:277] + "..."
    ...
    try:
        ...
        # If editing fails (e.g., message is identical), send a new message
        try:
            await bot.send_message(cb.from_user.id, text, reply_markup=back_kb)
        except Exception as e2:
...
async def show_all_customer_reviews(cb: types.CallbackQuery, state: FSMContext):
    ...
    if not reviews_from_db:
        ...
        for review in reviews_from_db:
            ...
            if order_items:
                text += f"  🛒 Basket: "
                item_names = [f"{str(item.get('name', 'Item'))[:40]} ({str(item.get('size', ''))[:20]})" for item in order_items[:2]]
                if len(order_items) > 2:
                    item_names.append(f"and {len(order_items)-2} more...")
