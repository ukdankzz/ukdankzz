...
class DatabaseManager:
    ...
    def migrate_existing_products(self, products_dict: Dict):
        ...
        try:
            conn = self.get_connection()
            with conn.cursor() as cur:
                cur.execute("SELECT COUNT(*) AS cnt FROM menu_categories")
                row = cur.fetchone()
                if (row['cnt'] if isinstance(row, dict) else row[0]) > 0:
                    logger.info("Menu already migrated, skipping")
                    return
        except Exception as exc:
            logger.error(f"❌ Error checking migration: {exc}")
            return
        finally:
            if conn:
                self.put_connection(conn)
        ...
        for order, (cat_name, cat_products) in enumerate(products_dict.items()):
            ...
            for prod_name, tiers in cat_products.items():
                ...
                for tier_order, (size, price) in enumerate(tiers):
                    self.add_pricing_tier(prod_id, size, float(price), tier_order)
