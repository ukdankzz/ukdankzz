...
def status():
    ...
    return {
        ...
        "total_broadcast_users": system_status['total_broadcast_users']
    }
...
def make_url_button(text: str, url: str) -> InlineKeyboardButton | None:
    ...
    if not safe_url:
        logger.warning(f'Blocked unsafe button URL: {url}')
        return None
    return InlineKeyboardButton(text=text[:64], url=safe_url)
...
def get_default_public_link_button_data() -> list[list[dict[str, str]]]:
    return [
        [{'text': '📞 Contact Me', 'url': 'https://t.me/ogukdankzz'}],
        [{'text': 'Main gc', 'url': 'https://t.me/+BVxAm2gq7Ek2YWM0'},
         {'text': 'Visual gc', 'url': 'https://t.me/+aLPTCkZhDH1iZTBk'}],
        [{'text': 'Back up gc', 'url': 'https://t.me/+XZwZqNXUuuc1NTk0'}],
    ]


def normalize_public_link_button_data(rows_source: Any) -> list[list[dict[str, str]]]:
    ...
    for row in rows_source:
        ...
        for item in row[:4]:
            ...
            normalized_row.append({'text': label[:64], 'url': safe_url})
...
def load_public_link_button_data() -> list[list[dict[str, str]]]:
    rows_source: Any = get_default_public_link_button_data()

    if os.path.exists(PUBLIC_LINK_BUTTONS_FILE):
        try:
            with open(PUBLIC_LINK_BUTTONS_FILE, 'r', encoding='utf-8') as f:
                file_data = json.load(f)
            normalized = normalize_public_link_button_data(file_data)
            if normalized:
                return normalized
        except Exception as e:
            logger.warning(f'Invalid public link button file, using fallback: {e}')

    raw = os.getenv('PUBLIC_LINK_BUTTONS', '').strip()
    if raw:
        try:
            parsed = json.loads(raw)
            normalized = normalize_public_link_button_data(parsed)
            if normalized:
                return normalized
        except Exception as e:
            logger.warning(f'Invalid PUBLIC_LINK_BUTTONS JSON, using defaults: {e}')

    return normalize_public_link_button_data(rows_source)


def build_public_link_button_rows(data: list[list[dict[str, str]]]) -> list[list[InlineKeyboardButton]]:
    rows: list[list[InlineKeyboardButton]] = []
    for row in data:
        button_row: list[InlineKeyboardButton] = []
        for item in row:
            btn = make_url_button(item['text'], item['url'])
            if btn:
...
def refresh_public_link_buttons() -> None:
    global PUBLIC_LINK_BUTTON_DATA, PUBLIC_LINK_BUTTON_ROWS
    PUBLIC_LINK_BUTTON_ROWS = build_public_link_button_rows(PUBLIC_LINK_BUTTON_DATA)


def save_public_link_button_data(data: list[list[dict[str, str]]]) -> bool:
    global PUBLIC_LINK_BUTTON_DATA
    try:
        normalized = normalize_public_link_button_data(data)
        with open(PUBLIC_LINK_BUTTONS_FILE, 'w', encoding='utf-8') as f:
            json.dump(normalized, f, ensure_ascii=False, indent=2)
        PUBLIC_LINK_BUTTON_DATA = normalized
        refresh_public_link_buttons()
        return True
    except Exception as e:
        logger.error(f'Failed to save public link buttons: {e}')
        return False


PUBLIC_LINK_BUTTON_DATA = load_public_link_button_data()
PUBLIC_LINK_BUTTON_ROWS: list[list[InlineKeyboardButton]] = []
refresh_public_link_buttons()
...
class PublicLinkStates(StatesGroup):
    waiting_for_new_link = State()
    waiting_for_edit_link = State()


def parse_public_link_input(text_value: str) -> tuple[str | None, str | None]:
    if '|' not in text_value:
        return None, None
    label, url = [part.strip() for part in text_value.split('|', 1)]
    safe_url = sanitize_button_url(url)
    if not label or not safe_url:
        return None, None
    return label[:64], safe_url


def build_public_links_admin_kb() -> InlineKeyboardMarkup:
    buttons: list[list[InlineKeyboardButton]] = [
        [InlineKeyboardButton(text="➕ Add Link", callback_data="admin_links_add")]
    ]

    for row_idx, row in enumerate(PUBLIC_LINK_BUTTON_DATA):
        for col_idx, item in enumerate(row):
            label = item.get('text', 'Link')[:32]
            buttons.append([
                InlineKeyboardButton(text=f"✏️ Edit {label}", callback_data=f"admin_link_edit_{row_idx}_{col_idx}"),
                InlineKeyboardButton(text=f"🗑 Delete", callback_data=f"admin_link_delete_{row_idx}_{col_idx}")
            ])
...
def format_public_links_admin_text() -> str:
    text = "🔗 Manage Public Links\n\n"
    if not PUBLIC_LINK_BUTTON_DATA:
        text += "No public link buttons set.\n\n"
    else:
        for row_idx, row in enumerate(PUBLIC_LINK_BUTTON_DATA, start=1):
            text += f"Row {row_idx}\n"
            for col_idx, item in enumerate(row, start=1):
                label = str(item.get('text', 'Link'))
                url = str(item.get('url', ''))
                text += f"{row_idx}.{col_idx} {label} -> {url}\n"
            text += "\n"
    text += "Use the buttons below to add, edit, or remove link buttons."
    return text
...
async def admin_panel(message: types.Message, state: FSMContext):
    ...
    total_orders = 0
    pending_orders = 0
    total_users = len(db.get_broadcast_users()) if db else len(all_users)
    ...
    admin_kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🔗 Manage Public Links", callback_data="admin_links")],
...
async def view_pending_orders(cb: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if (cb.message and 
            ...
            await cb.answer()
            await bot.send_message(cb.from_user.id, text, reply_markup=kb, parse_mode=None)
    except Exception as e:
...
async def admin_broadcast_start(cb: types.CallbackQuery, state: FSMContext):
    ...
    broadcast_total = len(db.get_broadcast_users()) if db else len(all_users)
    text += f"👥 Total users: {broadcast_total}\n\n"
...
async def admin_broadcast_send(message: types.Message, state: FSMContext):
    ...
    broadcast_total = len(db.get_broadcast_users()) if db else len(all_users)
    result_text += f"👥 Total users: {broadcast_total}\n"
...
async def admin_stats(cb: types.CallbackQuery, state: FSMContext):
    ...
    total_orders = db.get_total_orders_count() if db else 0
...
@uncrashable
async def admin_links_panel(cb: types.CallbackQuery, state: FSMContext):
    await cb.answer()
    track_user(cb.from_user)
    ...
    await state.clear()
    text = format_public_links_admin_text()
    kb = build_public_links_admin_kb()
...
@uncrashable
async def admin_links_add_prompt(cb: types.CallbackQuery, state: FSMContext):
    await cb.answer()
    track_user(cb.from_user)
    ...
    await state.set_state(PublicLinkStates.waiting_for_new_link)
    await cb.message.answer(
        "Send the new button in this format:\n\n"
        "Button Name | https://t.me/yourlink\n\n"
        "Examples:\n"
        "Support | https://t.me/ogukdankzz\n"
        "Channel | https://t.me/examplechannel",
        parse_mode=None
    )


@dp.message(PublicLinkStates.waiting_for_new_link)
@uncrashable
async def admin_links_add_save(message: types.Message, state: FSMContext):
    if not message.from_user or not is_admin(message.from_user.id):
        await message.answer("❌ Access denied.")
        return

    label, url = parse_public_link_input(message.text or "")
    if not label or not url:
        await message.answer("❌ Invalid format. Use:\nButton Name | https://t.me/yourlink", parse_mode=None)
        return

    new_data = [list(row) for row in PUBLIC_LINK_BUTTON_DATA]
    new_data.append([{'text': label, 'url': url}])

    if not save_public_link_button_data(new_data):
        await message.answer("❌ Failed to save public link.")
        return

    await state.clear()
    await message.answer("✅ Link button added.", reply_markup=build_public_links_admin_kb(), parse_mode=None)
    await message.answer(format_public_links_admin_text(), reply_markup=build_public_links_admin_kb(), parse_mode=None)


@dp.callback_query(F.data.startswith("admin_link_delete_"))
@uncrashable
async def admin_links_delete(cb: types.CallbackQuery, state: FSMContext):
    await cb.answer()
    track_user(cb.from_user)
    ...
    try:
        _, _, _, row_idx, col_idx = cb.data.split("_")
        row_i = int(row_idx)
        col_i = int(col_idx)
        new_data = [list(row) for row in PUBLIC_LINK_BUTTON_DATA]

        if row_i >= len(new_data) or col_i >= len(new_data[row_i]):
            await cb.answer("❌ Link not found.", show_alert=True)
            return

        del new_data[row_i][col_i]
        new_data = [row for row in new_data if row]

        if not save_public_link_button_data(new_data):
            await cb.answer("❌ Failed to delete link.", show_alert=True)
            return

        await cb.answer("✅ Link removed.", show_alert=True)
        await cb.message.edit_text(format_public_links_admin_text(), reply_markup=build_public_links_admin_kb(), parse_mode=None)
    except Exception as e:
        logger.error(f"Error deleting public link: {e}")
        await cb.answer("❌ Failed to delete link.", show_alert=True)


@dp.callback_query(F.data.startswith("admin_link_edit_"))
@uncrashable
async def admin_links_edit_prompt(cb: types.CallbackQuery, state: FSMContext):
    await cb.answer()
    track_user(cb.from_user)
    ...
    try:
        ...
        row_i = int(row_idx)
        col_i = int(col_idx)
        item = PUBLIC_LINK_BUTTON_DATA[row_i][col_i]

        await state.set_state(PublicLinkStates.waiting_for_edit_link)
        await state.update_data(edit_public_link_row=row_i, edit_public_link_col=col_i)
        await cb.message.answer(
            "Send the updated button in this format:\n\n"
            "Button Name | https://t.me/yourlink\n\n"
            f"Current: {item.get('text', '')} | {item.get('url', '')}",
            parse_mode=None
        )
    except Exception as e:
        logger.error(f"Error preparing public link edit: {e}")
        await cb.answer("❌ Failed to open edit prompt.", show_alert=True)


@dp.message(PublicLinkStates.waiting_for_edit_link)
@uncrashable
async def admin_links_edit_save(message: types.Message, state: FSMContext):
    if not message.from_user or not is_admin(message.from_user.id):
        await message.answer("❌ Access denied.")
        return
    ...
    data = await state.get_data()
    row_i = int(data.get("edit_public_link_row", -1))
    col_i = int(data.get("edit_public_link_col", -1))
    new_data = [list(row) for row in PUBLIC_LINK_BUTTON_DATA]

    if row_i < 0 or row_i >= len(new_data) or col_i < 0 or col_i >= len(new_data[row_i]):
        await state.clear()
        await message.answer("❌ Link not found.", parse_mode=None)
        return

    new_data[row_i][col_i] = {'text': label, 'url': url}

    if not save_public_link_button_data(new_data):
        await message.answer("❌ Failed to save updated link.", parse_mode=None)
        return

    await state.clear()
    await message.answer("✅ Link button updated.", parse_mode=None)
    await message.answer(format_public_links_admin_text(), reply_markup=build_public_links_admin_kb(), parse_mode=None)
...
async def admin_menu_management(cb: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        menu_data = db.get_dynamic_menu()
        total_products = sum(len(products_dict) for products_dict in menu_data.get('menu', {}).values())
...
async def select_category_for_product(cb: types.CallbackQuery, state: FSMContext):
    ...
    text += "Step 2: Type the product name or multiple variation names:\n"
    text += "• Single: `Blueberry`\n"
    text += "• Multiple at once: `Blueberry, Raspberry, Apple`\n\n"
    text += "All names entered together will be created in this category with the same pricing tiers."
...
async def add_product_name(message: types.Message, state: FSMContext):
    ...
    raw_names = message.text.strip()
    product_names = [name.strip() for name in raw_names.replace("\n", ",").split(",") if name.strip()]
    if not product_names:
        await message.answer("❌ Please enter at least one valid product name.")
        return
    if len(set(name.lower() for name in product_names)) != len(product_names):
        await message.answer("❌ Duplicate variation names detected. Please remove duplicates and try again.")
        return
    
    # Store product name(s) in state
    await state.update_data(new_product_names=product_names, new_product_name=product_names[0])
    ...
    if len(product_names) == 1:
        text += f"Product: **{product_names[0]}**\n\n"
    else:
        text += "Variations to add:\n" + "\n".join([f"• {name}" for name in product_names]) + "\n\n"
...
async def add_product_pricing(message: types.Message, state: FSMContext):
    ...
    if is_edit:
        ...
        # Add new product(s) - get category_id from state
        category_id = data.get('new_product_category_id')
        product_names = data.get('new_product_names') or [product_name]
        if not category_id:
        ...
        for product_variant_name in product_names:
            result = db.add_product(category_id, product_variant_name)
            if not result['success']:
                failed_products.append(f"• {product_variant_name}: {result.get('error', 'Unknown error')}")
                continue
            ...
            for tier_order, (size, price) in enumerate(pricing_tiers):
                ...
                if not tier_result['success']:
                    logger.error(f"❌ Error adding pricing tier for {product_variant_name}: {tier_result.get('error')}")
            created_products.append(product_variant_name)
        
        result = {'success': len(created_products) > 0, 'error': "\n".join(failed_products)}
    ...
    if result['success']:
        ...
        if is_edit:
            summary_name = f"**{product_name}** ({category})"
        else:
            created_products = data.get('new_product_names') or [product_name]
            if len(created_products) == 1:
                summary_name = f"**{created_products[0]}** ({category})"
            else:
                summary_name = f"**{len(created_products)} products** ({category})\n" + "\n".join([f"• {name}" for name in created_products])
        
        reply_text = success_msg + f"{summary_name}\n\nPricing:\n{pricing_text}"
        if not is_edit and result.get('error'):
            reply_text += "\n\n⚠️ Failed variations:\n" + result['error']
...
@dp.callback_query(F.data == "remove_inactive_product")
async def remove_inactive_product_start(cb: types.CallbackQuery, state: FSMContext):
    await cb.answer()
    track_user(cb.from_user)
    if not is_admin(cb.from_user.id):
        await cb.answer("❌ Access denied.", show_alert=True)
    ...
    inactive_products = [product for product in all_products if not product.get('active')]

    compact_inactive_products = [
        {
    ...
    await state.update_data(inactive_products_for_delete=compact_inactive_products)

    text = "🗑️ *Delete Inactive Product*\n\n"
    text += "Select an inactive product to permanently delete without reactivating it:\n\n"

    buttons = []
    if not compact_inactive_products:
        text += "No inactive products available to delete."
        buttons.append([InlineKeyboardButton(text="⬅️ Back", callback_data="admin_menu")])
    else:
        for idx, product in enumerate(compact_inactive_products):
            button_text = f"🗑️ {product['product']} ({product['category']})"
            if len(button_text) > 60:
                button_text = button_text[:57] + "..."
            buttons.append([InlineKeyboardButton(
                text=button_text,
                callback_data=f"confirm_remove_inactive_idx_{idx}"
            )])
        buttons.append([InlineKeyboardButton(text="⬅️ Back", callback_data="admin_menu")])
    ...
    try:
        ...
        logger.error(f"❌ Error showing inactive products: {e}")

@dp.callback_query(F.data.startswith("confirm_remove_inactive_idx_"))
async def confirm_remove_inactive_product(cb: types.CallbackQuery, state: FSMContext):
    await cb.answer()
    track_user(cb.from_user)
    if not is_admin(cb.from_user.id):
        await cb.answer("❌ Access denied.", show_alert=True)
    ...
    try:
        idx = int(cb.data.replace("confirm_remove_inactive_idx_", ""))
    except ValueError:
        await cb.answer("❌ Invalid product selection", show_alert=True)
        return

    state_data = await state.get_data()
    inactive_products = state_data.get('inactive_products_for_delete', [])

    if idx < 0 or idx >= len(inactive_products):
        await cb.answer("❌ Product list expired. Please reopen delete inactive products.", show_alert=True)
        return

    selected = inactive_products[idx]
    ...
    if result['success']:
        ...
        products = menu_data['menu']
        descriptions.update(menu_data['descriptions'])
        await cb.answer(f"✅ Permanently deleted inactive product '{product}'!", show_alert=True)
        await remove_inactive_product_start(cb, state)
    else:
        await cb.answer(f"❌ Error: {result['error']}", show_alert=True)
...
async def show_product_reviews(cb: types.CallbackQuery, state: FSMContext):
    ...
    user_is_admin = is_admin(cb.from_user.id)
    ...
    if not product_specific_reviews:
        ...
        for review in product_specific_reviews:
            ...
            safe_review_text = str(review_text)
            if len(safe_review_text) > 160:
                safe_review_text = safe_review_text[:157] + "..."
            text += f"  \"{safe_review_text}\"\n"
            text += f"  📅 {date}\n\n"
    ...
    try:
        if cb.message and hasattr(cb.message, 'edit_text'):
            await cb.message.edit_text(text, reply_markup=back_kb, parse_mode=None)
    except Exception as e:
        print(f"Error showing product reviews: {e}")
        # If editing fails (e.g., message is identical), send a new message
        try:
            await bot.send_message(cb.from_user.id, text, reply_markup=back_kb, parse_mode=None)
        except Exception as e2:
...
async def show_all_customer_reviews(cb: types.CallbackQuery, state: FSMContext):
    ...
    if not reviews_from_db:
        ...
        for review in reviews_from_db:
            ...
            if order_items:
                text += f"  🛒 Basket: "
                item_names = [f"{str(item.get('name', 'Item'))[:28]} ({str(item.get('size', ''))[:12]})" for item in order_items[:1]]
                if len(order_items) > 1:
                    item_names.append(f"and {len(order_items)-1} more...")
